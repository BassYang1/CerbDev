    ...
    jQuery("#DataGrid").jqGrid({
        ...
        colModel: [
            ...
            { name: 'OperPermissions', index: 'OperPermissions', width: 10, hidden: true, viewable: false, editrules: { edithidden: false } },
            {
                ...
                width: 220,
                edittype: 'select',
                editoptions: { value: "2:" + getlbl("tool.OperPermDescVal2") + ";1:" + getlbl("tool.OperPermDescVal1") },
                formoptions: { elmsuffix: "<span id='spanShowOperPermMsg' style='color:red'></span>", rowpos: 2, colpos: 2 }, //2:一般职员;1:系统管理员
            },
    ...
    jQuery("#DataGrid").jqGrid('navGrid', '#DataGrid_toppager',
        {
            ...
            refreshtext: strrefreshtext,
            viewtext: strviewtext,
...
function InitForm() {
    var deptListHtml = GetDepartmentHtml();
...
function InitEditForm(oper, userId) {
    ...
    var deptListHtml = GetDepartmentHtml();
    deptListHtml = "<select id='SelDepartment' name='SelDepartment' style='width:160px' class='FormElement ui-widget-content ui-corner-all'><option value='' selected></option>" + deptListHtml + "</select>";
    $("#tr_Name").children().eq(1).html("&nbsp;" + deptListHtml + "&nbsp;<select id = 'SelEmployees' class='FormElement ui-widget-content ui-corner-all' style='width:160px'></select><font color=#FF0000>*</font>");
    //绑定部门下拉框onChange事件
