...
dim a
set a=new JSONClass
strSQL = "SELECT L.AskForLeaveId,D.DepartmentName,E.Name,L.AskForLeaveType, Note, AllDay, convert(varchar(19),StartTime,121) as StartTime, convert(varchar(19),EndTime,121) as EndTime, Status "
strSQL = strSQL & "FROM AttendanceAskForLeave L "
strSQL = strSQL & "INNER JOIN Employees E ON E.EmployeeId = L.EmployeeId "
strSQL = strSQL & "INNER JOIN Departments D ON E.DepartmentCode = D.DepartmentCode "
strSQL = strSQL & "WHERE L.AskForLeaveType <> '" & getEmpLbl("Emp_Trip") & "' "
