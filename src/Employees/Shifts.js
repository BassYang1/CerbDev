...
jQuery("#DataGrid").jqGrid({
		...
		colModel :[
			...
			{name:'FirstOnDuty',index:'FirstOnDuty',editable:true,hidden:true,align:'center',editrules:{required:false,edithidden:true},
				edittype:'select', editoptions: {value:'0:' + getlbl("hr.OnThatDay")},
...
function setFormStyle(){
    $("#tr_ShiftName>td").addClass("ui-border ui-NoneRightBorder");
    $("#tr_Night>td").addClass("ui-border ui-NoneRightBorder");
    $("#Night").css({"margin-top":"5px"});
    $("#tr_StretchShift>td").addClass("ui-border ui-NoneRightBorder");
    $("#StretchShift").css({"margin-top":"5px"});
    $("#Degree").css({"width":"120px"});
    $("#tr_ShiftName>td:last,#tr_Night>td:last,#tr_StretchShift>td:last").addClass("ui-border-R");
}

function bindFormEvent(rowObject){
