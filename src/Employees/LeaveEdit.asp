...
Select Case strOper
	...
	Case "del": 'Delete Record
		...
		strSQL = strSQL + " insert into FlowStepDetail(FlowType, FlowDataId, StepId, Transactorid,Transactor, Operation, OperateTime, Postil) select '" + getEmpLbl("FlowType_Leave_0") + "', AskForLeaveId, 0, L.EmployeeId, (select Name from Employees where EmployeeId= L.EmployeeId) AS TransactorName, '" + strStatus+  "', getdate(), '' from AttendanceAskForLeave L where left(L.status, 1) = '2' and askforleaveid in (" + CStr(strRecordID) + ");"
End Select
