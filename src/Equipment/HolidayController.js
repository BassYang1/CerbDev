...
//获取操作权限
var role = GetOperRole("HolidayController");
...
jQuery("#DataGrid").jqGrid({
	...
	colModel :[
		...
		{name:'Location',index:'Location',stype:"text",align:'center', searchoptions:{ sopt:["eq","ne",'cn','nc']},},
		{name:'IP',index:'IP',search:false,stype:"text",align:'center', searchoptions:{ sopt:["eq","ne",'cn','nc']},},
	...
	caption:getlbl("con.ConHoliday"),//"设备假期表",
	imgpath:'/images',
	multiselect: false,
	rowNum:irowNum,
	...
	subGridRowExpanded: function(subgrid_id, row_id) {
		...
		$("#"+subgrid_table_id).jqGrid({
			url:"HolidayControllerDetailList.asp?ControllerId="+row_id,
			datatype: "json",
			//colNames:['序号','TemplateId','假期名称','OperType'],
			colNames:[getlbl("con.SN"),'TemplateId',getlbl("con.HolidayName"),'OperType'],
			colModel :[
				{name:'HolidayNumber',index:'HolidayNumber',width:40,editrules:{edithidden:false},search:false,sortable:false},  
		...
		function ShowHolidaySelect(HolidayNumber)
		{
			var SelectHolidayID = subgrid_table_id+"_SelHoliday_"+HolidayNumber;
			var htmlObj = $.ajax({url:"../Common/GetHoliday.asp?type=all&SelectID="+SelectHolidayID,async:false});
			//$("#"+subgrid_table_id).setRowData(HolidayNumber, { TemplateName:htmlObj}); 
			//$("#"+subgrid_table_id).children().children().eq(HolidayNumber).children().eq(2).html(htmlObj.responseText);
			//20150722 改为以下方法：
			$("#"+subgrid_table_id + " tr[id='"+HolidayNumber+"']").children().eq(2).html(htmlObj.responseText);
	
			var SelVal = $("#"+subgrid_table_id).jqGrid("getRowData",HolidayNumber).TemplateId;
			var count=$("#"+SelectHolidayID+" option").length;
			for(var i=0;i<count;i++){
				if($("#"+SelectHolidayID).get(0).options[i].value == SelVal){  
			...
			$("#"+SelectHolidayID).change(function(){
				...
				$("#"+subgrid_table_id).setRowData( HolidayNumber, { TemplateId:$("#"+SelectHolidayID).val()}); 
				$("#"+subgrid_table_id+"_btnSubmit").removeClass('ui-state-disabled'); 
				$("#"+subgrid_table_id+"_btnCancel").removeClass('ui-state-disabled'); 
			})
		}
		...
		function subGridAdd(){
			...
			var htmlObj = $.ajax({url:"GetControllerHolidayMaxHolidayCode.asp?ControllerId="+row_id+"&nd="+getRandom(),async:false});
		...
		function subGridSubmit(){
			...
			for(var i = 0; i < len; i++){
				var ret = $("#"+subgrid_table_id).jqGrid("getRowData", rowIds[i]);
				var SelectHolidayID = subgrid_table_id+"_SelHoliday_"+rowIds[i];
				var TemplateId = $("#"+SelectHolidayID).val();
				var TemplateName = "";
				//下拉框存在
				if($("#"+SelectHolidayID).length>0)
				{
					...
					if(TemplateId == undefined){
						...
						TemplateName=$("#"+SelectHolidayID).find("option:selected").text();
					}
				}
				if(ret.OperType == "add"){
					if(TemplateId != undefined){
					...
					if(TemplateId != undefined){
						...
						//没有修改过的数据，如果不为空，也重新提交到后台，主要目的是判断是否选择的假期表有重复
...
jQuery("#DataGrid").jqGrid('navGrid','#DataGrid_toppager', 
	...
	if(isync){
		$("#DataGrid").jqGrid('navGrid',"#DataGrid_toppager_left").jqGrid('navButtonAdd',"#DataGrid_toppager_left",{
			...
			title:getlbl("con.SyncHolidayTitle"),//"将假期表数据同步到设备",
			id:"DataGrid_btnSync",
...
function GetSyncStatus(){
	...
	$.ajax({
		type: 'Post',
		url: 'GetSyncStatus.asp?type=holiday',
...
function ExportData(){
	$("#divExport").load("../Tools/ExportDataUI.asp?nd="+getRandom()+"&exportType=holiday");
	$("#divExport").show();
}
