...
a.Sqlstring="select ControllerId,ControllerNumber,Location,IP,WorkType,ServerIP from Controllers"&strWhere&" "&"order by "& sidx & " " & sord
set a.dbconnection=conn

strExportSql="select C.ControllerId,ControllerNumber,ControllerName,Location,IP,WorkType,ServerIP,(Case when ISNULL(CDS.SyncStatus,0)=1 then '"&GetEquLbl("SyncEd")&"' else '"&GetEquLbl("UnSync")&"' end) as SyncStatus from Controllers C left join  ControllerDataSync CDS on (C.ControllerId=CDS.ControllerId and CDS.SyncType='register')"&strExportSql&" "&"order by "& Replace(sidx,"ControllerId","C.ControllerId") & " " & sord
