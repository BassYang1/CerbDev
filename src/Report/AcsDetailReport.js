...
$(document).ready(function(){  
	...
	InitForm();
	//获取操作权限
	var role = GetOperRole("AcsDetailReport");
...
jQuery("#DataGrid").jqGrid({
		...
		caption:getlbl("rep.AcsDetailRport"),//"进出明细报表",
		imgpath:'/images',
		multiselect: false,
		rowNum:irowNum,
		rowList:[10,16,20,30],
...
function gridReload(){
	$("#DataGrid").jqGrid('setGridParam',{url:"AcsDetailReportList.asp",page:1,}).trigger("reloadGrid");
}

}); 
function ExportData(){
	$("#divExport").load("../Tools/ExportDataUI.asp?nd="+getRandom()+"&exportType=acsdetail");
	$("#divExport").show();
}
function Search(){
...
function SearchSubmit(){
	...
	$("#DataGrid").jqGrid('setGridParam',{url:"AcsDetailReportList.asp?search=true&searchField="+strsearchField+"&searchOper="+strsearchOper+"&searchString="+encodeURI(strsearchString),page:1,}).trigger("reloadGrid");
