...
$(document).ready(function(){  
	...
	InitForm();
	//获取操作权限
	var role = GetOperRole("AttendDetailReport");
...
jQuery("#DataGrid").jqGrid({
		url:'AttendDetailReportList.asp',
		datatype: "json",
		...
		caption:getlbl("rep.AttendDetailReport"),//"考勤明细报表",
		imgpath:'/images',
		multiselect: false,
		rowNum:irowNum,
		rowList:[10,16,20,30],
...
function gridReload(){
	$("#DataGrid").jqGrid('setGridParam',{url:"AttendDetailReportList.asp",page:1,}).trigger("reloadGrid");
}

}); 

function ExportData(){
	$("#divExport").load("../Tools/ExportDataUI.asp?nd="+getRandom()+"&exportType=attenddetail");
	$("#divExport").show();
}
function Search(){
...
function SearchSubmit(){
	...
	$("#DataGrid").jqGrid('setGridParam',{url:"AttendDetailReportList.asp?search=true&searchField="+strsearchField+"&searchOper="+strsearchOper+"&searchString="+encodeURI(strsearchString),page:1,}).trigger("reloadGrid");
